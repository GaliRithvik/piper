...
# 5. Format a file
cargo run -- fmt myfile.piper
```

**VS Code users** — install syntax highlighting + run support:
```bash
cp -r piper-vscode ~/.vscode/extensions/piper-lang-0.9.0
```
Then reload VS Code (`Cmd+Shift+P` → **Reload Window**) and open any `.piper` file.
...
- **VS Code extension** — syntax highlighting, run shortcut, format-on-save for `.piper` files
...
│   ├── index.html                # Playground UI — collapsible syntax sidebar + editor + output
...
│
├── piper-vscode/                 # VS Code extension — syntax highlighting + run + format
│   ├── package.json              # Extension manifest (contributes languages, grammars, commands)
│   ├── language-configuration.json  # Auto-indent, bracket matching, comment toggling
│   ├── extension.js              # Run File (Cmd+Shift+R), Format File (Cmd+Shift+I), format-on-save
│   └── syntaxes/
│       └── piper.tmLanguage.json # TextMate grammar — keywords, strings, operators, classes, fns
│
├── examples/
...
Full IDE support for `.piper` files — syntax highlighting, run shortcut, formatter.
...
**Windows:**
```cmd
xcopy /E /I piper-vscode "%USERPROFILE%\.vscode\extensions\piper-lang-0.9.0"
```
...
| Shortcut | Action |
|---|---|
| `Cmd+Shift+R` | Run current `.piper` file in terminal |
| `Cmd+Shift+I` | Format current file with `piper fmt` |
| `Shift+Alt+F` | VS Code "Format Document" (also triggers `piper fmt`) |

Syntax highlighting covers keywords, strings, f-strings, `\|>`, `??`, `?.`, classes, functions, numbers, and constants. Auto-indent triggers on `:` lines. Bracket/quote matching is enabled.

### Formatter

```bash
piper fmt myfile.piper    # format in place
```

Applies: trailing whitespace removal, tab → 4-space normalisation, blank line capping, blank line before top-level `fn`/`class`, space after `#` in comments, space after `,`.
...
- Added **formatter** — `piper fmt file.piper` auto-formats spacing, indentation, blank lines, and comment style
...
- Playground upgraded to full **syntax reference sidebar** — collapsible sections covering every language feature, each with a runnable snippet
